...
if ($status_param === 'saved')   $success = 'Feature card saved successfully!';
if ($status_param === 'deleted') $success = 'Feature card deleted successfully!';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['delete_id'] ?? '') === '') {
    if (!verifyCsrfToken()) {
        ...
        if (empty($title)) {
            ...
            if ($action === 'add') {
                $stmt = $db->prepare("INSERT INTO homepage_features (title, description, icon, display_order, is_active, updated_by) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("sssiii", $title, $description, $icon, $display_order, $is_active, $_SESSION['admin_id']);
                if ($stmt->execute()) {
                    logActivity('create', 'homepage_features', $db->insert_id, "Added feature: {$title}");
                    redirect('homepage-features.php?status=saved');
                } else {
                    $error = 'Error: ' . $stmt->error;
                }
            } elseif ($action === 'edit' && $id) {
                $stmt = $db->prepare("UPDATE homepage_features SET title = ?, description = ?, icon = ?, display_order = ?, is_active = ?, updated_by = ? WHERE id = ?");
                $stmt->bind_param("sssiiii", $title, $description, $icon, $display_order, $is_active, $_SESSION['admin_id'], $id);
                if ($stmt->execute()) {
                    logActivity('update', 'homepage_features', $id, "Updated feature: {$title}");
                    redirect('homepage-features.php?status=saved');
                } else {
                    $error = 'Error: ' . $stmt->error;
                }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['delete_id'] ?? '') !== '') {
    if (!verifyCsrfToken()) {
        ...
        if ($stmt->execute()) {
            logActivity('delete', 'homepage_features', $del_id, 'Deleted homepage feature');
            redirect('homepage-features.php?status=deleted');
        } else {
            $error = 'Error deleting feature: ' . $stmt->error;
        }
    }
}
...
$edit_feature = null;
if ($action === 'edit' && $id) {
    $stmt = $db->prepare("SELECT * FROM homepage_features WHERE id = ?");
...
// Get all features for list (with pagination + filtering + search)
$all_features = [];
$total_features = 0;
$total_pages = 1;
$current_page_num = 1;
...
if ($action === 'list') {
    ...
    // Total count
    $count_stmt = $db->prepare("SELECT COUNT(*) as total FROM homepage_features $where_sql");
    if ($types && $count_stmt) {
        $count_stmt->bind_param($types, ...$params);
    }
    if ($count_stmt) {
        $count_stmt->execute();
        $total_features = $count_stmt->get_result()->fetch_assoc()['total'];
    }
    $total_pages = max(1, ceil($total_features / $per_page));
    ...
    // List query
    $list_stmt = $db->prepare("SELECT * FROM homepage_features $where_sql ORDER BY display_order, id LIMIT ? OFFSET ?");
    $list_params = $params;
    $list_types = $types . 'ii';
...
$st = $db->query("SELECT COUNT(*) total, SUM(is_active) active FROM homepage_features");
if ($st) {
    $row = $st->fetch_assoc();
...
// Feature cards render in the "Why Choose" section on the homepage
$view_url = '../index.php#features';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Homepage Features - <?php echo SITE_NAME; ?></title>
...
<body class="bg-gray-100">
    ...
    <div class="relative lg:ml-64 p-4 lg:p-8">
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div>
                    <h1 class="text-3xl font-bold text-gray-800 flex items-center gap-3">
                        <i class="fas fa-star text-primary"></i> Homepage Features
                    </h1>
                    <p class="text-sm text-gray-500 mt-1">Manage the "Why Choose Tupi Supreme?" feature cards on the homepage.</p>
                </div>
                <?php if ($action === 'list'): ?>
                ...
                <?php else: ?>
                <a href="homepage-features.php" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200 transition-colors inline-flex items-center justify-center">
        ...
        <?php if ($action === 'add' || $action === 'edit'): ?>
            <!-- Add/Edit Form -->
            <div class="p-6">
                <h2 class="text-2xl font-bold mb-1"><?php echo $action === 'add' ? 'Add New' : 'Edit'; ?> Feature Card</h2>
                ...
                <div class="mb-6 p-3 rounded-lg bg-blue-50 border border-blue-200 flex items-start gap-2">
                    ...
                    <p class="text-sm text-blue-800">Feature cards appear in a grid in the "Why Choose Tupi Supreme?" section of the public <a href="<?php echo $view_url; ?>" target="_blank" class="underline hover:text-blue-900">homepage</a>. Each card has a gradient icon tile, a title, and a short description. The section heading itself is editable under <a href="pages.php" class="underline hover:text-blue-900">Pages</a> (index → features_title / features_subtitle).</p>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <input type="text" name="title" id="title_input" required maxlength="200"
                                           value="<?php echo htmlspecialchars($edit_feature['title'] ?? ''); ?>"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
                                           placeholder="e.g., Premium Quality"
                                           oninput="updatePreview()">
                                    <p class="text-xs text-gray-400 mt-1">The feature heading on the card.</p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="description" id="description_input" rows="4"
                                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
                                              placeholder="e.g., Our 2mm granulated activated carbon products meet the highest industry standards..."
                                              oninput="updatePreview()"><?php echo htmlspecialchars($edit_feature['description'] ?? ''); ?></textarea>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="icon" id="icon_input" maxlength="100"
                                           value="<?php echo htmlspecialchars($edit_feature['icon'] ?? ''); ?>"
                                    ...
                                    <div class="mt-2 flex flex-wrap gap-1">
                                        ...
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-headset')">headset</button>
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-flask')">flask</button>
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-tint')">tint</button>
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-shield-alt')">shield</button>
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-truck')">truck</button>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input type="number" name="display_order" min="0"
                                               value="<?php echo htmlspecialchars($edit_feature['display_order'] ?? 0); ?>"
                                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary">
                                    ...
                                    <div class="flex items-center">
                                        <label class="flex items-center cursor-pointer select-none">
                                            <input type="checkbox" name="is_active" value="1"
                                                   <?php echo ($edit_feature && $edit_feature['is_active']) || !$edit_feature ? 'checked' : ''; ?>
                                                   class="sr-only peer">
                            ...
                            <div class="mt-8 flex flex-col sm:flex-row gap-3">
                                <button type="submit" class="bg-primary text-white px-6 py-2.5 rounded-lg hover:bg-secondary transition-colors inline-flex items-center justify-center">
                                    <i class="fas fa-save mr-2"></i><?php echo $action === 'add' ? 'Add Feature' : 'Save Changes'; ?>
                                </button>
                                <a href="homepage-features.php" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2.5 rounded-lg transition-colors inline-flex items-center justify-center">
                    ...
                    <div>
                        ...
                        <div class="bg-white border border-[#e6ece8] rounded-2xl p-8 max-w-sm text-center">
                            ...
                            <p id="preview_desc" class="text-[#7d8b84] leading-relaxed text-sm <?php echo empty($edit_feature['description'] ?? '') ? 'hidden' : ''; ?>"><?php echo htmlspecialchars($edit_feature['description'] ?? ''); ?></p>
                        </div>
                        <p class="text-xs text-gray-400 mt-2">The card style mirrors the feature cards on the public homepage.</p>
                    </div>
                </div>
            </div>
            ...
            function updatePreview() {
                var title = document.getElementById('title_input').value || 'Feature title';
        ...
        <?php else: ?>
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div class="p-4 bg-gray-50 border-b">
                    ...
                    <form method="GET" action="homepage-features.php" class="flex flex-col sm:flex-row gap-2">
                        ...
                        <a href="homepage-features.php" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-200 transition-colors text-sm inline-flex items-center justify-center">
                        ...
                        <?php if (empty($all_features)): ?>
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-gray-500 mb-2">No feature cards found.</p>
                                    <a href="?action=add" class="text-primary hover:underline text-sm"><i class="fas fa-plus mr-1"></i>Add new feature</a>
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($all_features as $feature): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 text-sm">
                                        <p class="font-medium text-gray-900"><?php echo htmlspecialchars($feature['title']); ?></p>
                                        <?php if ($feature['description']): ?>
                                            <p class="text-xs text-gray-400 mt-1"><?php echo htmlspecialchars(mb_strimwidth($feature['description'], 0, 90, '…')); ?></p>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-6 py-4 text-sm">
                                        <?php if ($feature['icon']): ?>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900"><?php echo (int)$feature['display_order']; ?></td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
                                        <a href="?action=edit&id=<?php echo $feature['id']; ?>" class="text-primary hover:text-secondary mr-3" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="<?php echo $view_url; ?>" target="_blank" class="text-gray-500 hover:text-gray-700 mr-3" title="View on site">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <form method="POST" action="homepage-features.php" class="inline" onsubmit="return confirm('Delete this feature card? This cannot be undone.');">
                                            <?php echo csrfTokenField(); ?>
            ...
            <div class="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <ul class="text-sm text-blue-800 space-y-1">
                    <li><a href="<?php echo $view_url; ?>" target="_blank" class="hover:underline"><i class="fas fa-external-link-alt mr-1"></i>View Features on Homepage</a></li>
                    <li><a href="pages.php" class="hover:underline"><i class="fas fa-file-alt mr-1"></i>Edit Homepage Section Headings</a></li>
