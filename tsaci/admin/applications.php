...
if ($status_param === 'saved')   $success = 'Application saved successfully!';
if ($status_param === 'deleted') $success = 'Application deleted successfully!';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['delete_id'] ?? '') === '') {
    ...
    if (empty($title)) {
        ...
        // Only one card can be the primary application
        if ($is_primary) {
            $db->query("UPDATE applications SET is_primary = 0");
        }

        if ($action === 'add') {
            $stmt = $db->prepare("INSERT INTO applications (title, description, icon, is_primary, display_order, is_active, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssiiii", $title, $description, $icon, $is_primary, $display_order, $is_active, $_SESSION['admin_id']);

            if ($stmt->execute()) {
                logActivity('create', 'applications', $db->insert_id, "Created application: {$title}");
                redirect('applications.php?status=saved');
            } else {
                $error = 'Error adding application: ' . $stmt->error;
            }
        } elseif ($action === 'edit' && $id) {
            $stmt = $db->prepare("UPDATE applications SET title = ?, description = ?, icon = ?, is_primary = ?, display_order = ?, is_active = ?, updated_by = ? WHERE id = ?");
            $stmt->bind_param("sssiiiii", $title, $description, $icon, $is_primary, $display_order, $is_active, $_SESSION['admin_id'], $id);

            if ($stmt->execute()) {
                logActivity('update', 'applications', $id, "Updated application: {$title}");
                redirect('applications.php?status=saved');
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['delete_id'] ?? '') !== '') {
    ...
    if ($stmt->execute()) {
        logActivity('delete', 'applications', $del_id, 'Deleted application');
        redirect('applications.php?status=deleted');
    } else {
        $error = 'Error deleting application: ' . $stmt->error;
    }
}

// Get application for edit
$edit_app = null;
if ($action === 'edit' && $id) {
...
// Get all applications for list (with pagination + filtering + search)
$all_apps = [];
$total_apps = 0;
$total_pages = 1;
$current_page_num = 1;
...
if ($action === 'list') {
    ...
    // Total count
    $count_sql = "SELECT COUNT(*) as total FROM applications a $where_sql";
    ...
    $total_pages = max(1, ceil($total_apps / $per_page));
    ...
    // List query
    $list_sql = "SELECT a.*, au.username as updated_by_name FROM applications a LEFT JOIN admin_users au ON a.updated_by = au.id $where_sql ORDER BY a.is_primary DESC, a.display_order, a.title LIMIT ? OFFSET ?";
...
$st = $db->query("SELECT COUNT(*) total, SUM(is_active) active FROM applications");
if ($st) {
    $row = $st->fetch_assoc();
...
// All applications render in the Applications grid on the products page
$view_url = '../products.php#applications';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="bg-gray-100">
    ...
    <div class="relative lg:ml-64 p-4 lg:p-8">
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div>
                    <h1 class="text-3xl font-bold text-gray-800 flex items-center gap-3">
                        <i class="fas fa-th-large text-primary"></i> Applications Management
                    </h1>
                    <p class="text-sm text-gray-500 mt-1">Manage the application cards in the "Applications" grid on the products page.</p>
                </div>
                <?php if ($action === 'list'): ?>
                <a href="?action=add" class="bg-primary text-white px-4 py-2 rounded-lg hover:bg-secondary transition-colors inline-flex items-center justify-center">
                    <i class="fas fa-plus mr-2"></i>Add New Application
                </a>
                <?php else: ?>
                <a href="applications.php" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200 transition-colors inline-flex items-center justify-center">
        ...
        <?php if ($action === 'add' || $action === 'edit'): ?>
            <!-- Add/Edit Form -->
            <div class="p-6">
                <h2 class="text-2xl font-bold mb-1"><?php echo $action === 'add' ? 'Add New' : 'Edit'; ?> Application</h2>
                ...
                <div class="mb-6 p-3 rounded-lg bg-blue-50 border border-blue-200 flex items-start gap-2">
                    ...
                    <p class="text-sm text-blue-800">Applications appear as cards in the <strong>Applications grid</strong> on the public <a href="<?php echo $view_url; ?>" target="_blank" class="underline hover:text-blue-900">products page</a>. They're shown in a 4-column grid, sorted by display order (left to right). One card can be marked <strong>Primary</strong> — it gets the dark featured style with a badge.</p>
                </div>
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div class="sm:col-span-2">
                                    ...
                                    <input type="text" name="title" id="title_input" required
                                           value="<?php echo htmlspecialchars($edit_app['title'] ?? ''); ?>"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
                                           placeholder="e.g., Municipal Water Treatment"
                                           oninput="updatePreview()">
                                    <p class="text-xs text-gray-400 mt-1">The card heading. Must be unique.</p>
                                </div>
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <input type="text" name="description" id="desc_input"
                                           value="<?php echo htmlspecialchars($edit_app['description'] ?? ''); ?>"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary"
                                           placeholder="e.g., Drinking water treatment for cities and towns"
                                           oninput="updatePreview()">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-3">
                                        <input type="text" name="icon" id="icon_input"
                                               value="<?php echo htmlspecialchars($edit_app['icon'] ?? ''); ?>"
                                    ...
                                    <div class="mt-2 flex flex-wrap gap-1">
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-building')">building</button>
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-industry')">industry</button>
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-hospital')">hospital</button>
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-flask')">flask</button>
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-gas-pump')">gas-pump</button>
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-home')">home</button>
                                        <button type="button" class="quick-pick" onclick="setIcon('fas fa-fish')">fish</button>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="display_order" min="0"
                                           value="<?php echo htmlspecialchars($edit_app['display_order'] ?? 0); ?>"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary focus:border-primary">
                                    <p class="text-xs text-gray-400 mt-1">Lower numbers appear first (left to right). The primary card always renders first. Use 10, 20, 30… to leave room for inserts.</p>
                                </div>
                            </div>

                            <div class="mt-6 space-y-4">
                                <label class="flex items-center cursor-pointer select-none">
                                    <input type="checkbox" name="is_primary" id="is_primary_input" value="1"
                                           <?php echo !empty($edit_app['is_primary']) ? 'checked' : ''; ?>
                                    ...
                                    <span class="ml-3 text-sm text-gray-700">Primary application <span class="text-gray-400">(dark featured card with badge — replaces the current primary)</span></span>
                                </label>

                                <label class="flex items-center cursor-pointer select-none">
                                    <input type="checkbox" name="is_active" value="1"
                                           <?php echo ($edit_app && $edit_app['is_active']) || !$edit_app ? 'checked' : ''; ?>
                                           class="sr-only peer">
                                    <span class="relative w-11 h-6 rounded-full bg-gray-300 transition-colors duration-300 ease-in-out
                                                 peer-checked:bg-primary peer-focus-visible:ring-2 peer-focus-visible:ring-primary/40 peer-focus-visible:ring-offset-2
                                                 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:w-5 after:h-5
                                                 ...
                                                 peer-checked:after:translate-x-5
                                                 hover:after:scale-110 active:after:scale-95"></span>
                                    <span class="ml-3 text-sm text-gray-700">Active <span class="text-gray-400">(shown on the products page)</span></span>
                                </label>
                            </div>
                            ...
                            <div class="mt-8 flex flex-col sm:flex-row gap-3">
                                <button type="submit" class="bg-primary text-white px-6 py-2.5 rounded-lg hover:bg-secondary transition-colors inline-flex items-center justify-center">
                                    <i class="fas fa-save mr-2"></i><?php echo $action === 'add' ? 'Add Application' : 'Save Changes'; ?>
                                </button>
                                <a href="applications.php" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2.5 rounded-lg transition-colors inline-flex items-center justify-center">
            ...
            function updateIconPreview(iconClass) {
                var cls = iconClass.trim() || 'fas fa-th-large';
            ...
            function updatePreview() {
                var title = document.getElementById('title_input').value || 'Application Title';
                var desc = document.getElementById('desc_input').value || 'Short description of this application.';
                var primary = document.getElementById('is_primary_input').checked;
        ...
        <?php else: ?>
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div class="p-4 bg-gray-50 border-b">
                    ...
                    <form method="GET" action="applications.php" class="flex flex-col sm:flex-row gap-2">
                        <div class="flex-1 flex flex-col sm:flex-row gap-2">
                            <select name="filter_status" class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-primary focus:border-primary">
                                ...
                                <option value="primary" <?php echo (($_GET['filter_status'] ?? '') === 'primary') ? 'selected' : ''; ?>>Primary only</option>
                            </select>
                        ...
                        <a href="applications.php" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-200 transition-colors text-sm inline-flex items-center justify-center">
                        ...
                        <?php if (empty($all_apps)): ?>
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-gray-500 mb-2">No applications found.</p>
                                    <a href="?action=add" class="text-primary hover:underline text-sm"><i class="fas fa-plus mr-1"></i>Add new application</a>
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($all_apps as $app): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="w-9 h-9 rounded-lg application-icon flex items-center justify-center">
                                            <i class="<?php echo htmlspecialchars($app['icon'] ?: 'fas fa-th-large'); ?> text-white text-sm"></i>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm font-medium text-gray-900"><?php echo htmlspecialchars($app['title']); ?></span>
                                    </td>
                                    <td class="px-6 py-4 max-w-xs">
                                        <span class="text-sm text-gray-500 truncate inline-block max-w-xs align-middle"><?php echo htmlspecialchars($app['description'] ?: '—'); ?></span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm text-gray-900"><?php echo (int)$app['display_order']; ?></span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="?action=edit&id=<?php echo $app['id']; ?>" class="text-primary hover:text-secondary mr-3" title="Edit">
