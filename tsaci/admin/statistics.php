...
// Get all statistics for list (with pagination)
$all_stats = [];
$total_stats = 0;
$total_pages = 1;
if ($action === 'list') {
    ...
    $count_result = $db->query("SELECT COUNT(*) as total FROM statistics");
    $total_stats = $count_result->fetch_assoc()['total'];
    ...
    $stmt = $db->prepare("SELECT s.*, au.username as updated_by_name FROM statistics s LEFT JOIN admin_users au ON s.updated_by = au.id ORDER BY s.display_order, s.label LIMIT ? OFFSET ?");
